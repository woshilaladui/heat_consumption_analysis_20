...
export const changeTimeChose = (timeChose) => ({
    type: constants.CHANGE_TIME_CHOSE_CSO,
    timeChose: timeChose
})
...
export const updateUpperData = (upperData) => ({//更新上表一的数据
    type: constants.UPDATE_UPPER_DATA_CSO,
    upperData: upperData,
});


export const updateBottomData = (bottomData) => ({//更新下表的数据
    type: constants.UPDATE_BOTTOM_DATA_CSO,
    bottomData: bottomData,
});
...
export function saveData({index = 0, tableType = 1, tableName, date, data, num = 1}) {
    return (dispatch) => {
        ZKSSave(
            URL.ZKS_SAVE,//http://127.0.0.1:8080/huayanshi/saveHuaYanShiData
            ...
            .then((response) => {
                if (num === 1)
                    ...
                    message.info('提交成功');
                    console.log("zhen de nan");
                    console.log( getZKSJsonSaveData({//获取封装好的请求头
                        tableType: tableType,//上表
                        tableName: tableName,
                        date: date,
                        index: index,
