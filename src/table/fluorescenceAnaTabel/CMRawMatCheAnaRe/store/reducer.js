...
const defaultState = fromJS({
    ...
    width:7,
    tableWidth:13,//表格总长度
    tableName: TableName.Limestone_CRM,//进厂石灰石原材料分析化学报告单
    allTime:[
...
export default (state = defaultState, action) => {
    ...
    switch (action.type) {
        case constants.UPDATE_DATA_CRM:
            return state.merge({
                'data': action.data,
                'requestFlag': false//切换页面时候不需要刷新数据了（当页面刷新的时候自动初始化为true）
            });
        case constants.CHANGE_TIME_CHOSE_CRM:
            return state.set('timeChose', action.timeChose);

        case constants.UPDATE_STANDARD_CRM://更新该表的标准
            return state.merge({
