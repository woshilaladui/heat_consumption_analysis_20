...
const defaultState = fromJS({
    ...
    width:7,//需要计算合计的长度
    tableWidth:7,//表格总长度
    tableName: TableName.Limestone_FMHs,//进厂石灰石原材料分析化学报告单
    allTime:[
...
export default (state = defaultState, action) => {
    ...
    switch (action.type) {
        case constants.UPDATE_DATA_RMA_FMHS_SHS:
            return state.merge({
                'data': action.data,
                'requestFlag': false//切换页面时候不需要刷新数据了（当页面刷新的时候自动初始化为true）
            });
        case constants.CHANGE_TIME_CHOSE_RMA_FMHS_SHS:
            return state.set('timeChose', action.timeChose);

        case constants.UPDATE_STANDARD_RMA_FMHS_SHS://更新该表的标准
            return state.merge({
