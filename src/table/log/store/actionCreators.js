...
import {
    requestGetAllLogRecordsByUsername,
    requestGetAllLogRecords,
    requestGetAllLogsByUsernameOrDateBetween
} from  '../../../http/request/RequestLogRecord'
import {
    Mark,
    Table,
    Department
...
export const updateData = ({data}) => ({
    type: constants.UPDATE_DATA_TFKT,
    data: data
});
const updateStartEndTime = (timeValueStart,timeValueEnd) => ({
    type: constants.UPDATE_START_END_TIME,
...
export const changeDataByUsername=(username,startTimeValue,endTimeValue)=>{
...
export const changeDataByTime=(time)=>{


    var startTimeTemp ='2001/01/01';
    var endTimeTemp   ='2025/01/01';
    if(time[0]!=null){
         startTimeTemp =time[0].format("YYYY/MM/DD")
         endTimeTemp =time[1].format("YYYY/MM/DD")
    }



    return(dispatch)=>{
        dispatch(updateStartEndTime(startTimeTemp,endTimeTemp));
        // requestGetAllLogsByUsernameOrDateBetween({
        //     startDate:time[0].format("YYYY/MM/DD"),endDate:time[1].format("YYYY/MM/DD")}
        // ).then((response)=>
        // {
        //     if(response['code'] === 0){
        //         //解析处理数据
